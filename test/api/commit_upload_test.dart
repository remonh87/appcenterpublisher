        ...
        setUp(() {
          when(client.patch(any,
              ...
              .thenAnswer((_) => Future.value(http.Response(
                  '{"release_id": "1234", "release_url": "http://foo1234"}',
                  200)));
        });

        test('It returns release id', () async {
          final response = await commitUpload(
              patch: client.patch,
              uploadId: '10',
              config: config,
              appName: 'test app');
      ...
      group('GIVEN api returns error', () {
        setUp(() {
          when(client.patch(any,
                  headers: anyNamed('headers'), body: anyNamed('body')))
              .thenAnswer((_) =>
                  Future.value(http.Response('something went wrong', 401)));
        });
        ...
        test('It returns $ApiOperationFailure', () async {
          ...
          expect(
              ...
              const ApiOperationFailure(
                  message: 'Api returned 401 message: something went wrong'));
        });
      });
    });
  });

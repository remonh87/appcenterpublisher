...
Future<DistributionResult> distributeToGroup({
  ...
  final response = await post(
      '$appcenterBaseUrl/${config.owner}/$appName/releases/$releaseId/groups',
      headers: headers,
      body: encoder.convert(distributionGroup.toJson()));
  ...
  if (response.statusCode == 201) {
    ...
    return DistributionResult.failure(ApiOperationFailure(
        message: 'Distributing failed error: ${response.body}'));
  }
}
